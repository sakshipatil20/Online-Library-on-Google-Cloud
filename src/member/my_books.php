		...
		<?php
			...
			else
			{
				...
				for($i=0; $i<$rows; $i++)
				{
					...
					if($isbn != NULL)
					{
						$query = $con->prepare("SELECT title, author, category, links FROM book WHERE isbn = ?;");
						...
						// echo "<td>".$innerRow[0]."</td>";
						echo "<td><a href='$link' target='_blank'>".$innerRow[3]."</td>";
						echo "</tr>";
					}
				}
				echo "</table><br />";
			...
			if(isset($_POST['b_return']))
			{
				...
				for($i=0; $i<$rows; $i++)
					if(isset($_POST['cb_book'.$i]))
					{
						...
						{
							$penalty = 500;
							$query = $con->prepare("SELECT credits FROM book WHERE isbn = ?;");
							$query->bind_param("s", $_POST['cb_book'.$i]);
							$query->execute();
							$credits = mysqli_fetch_array($query->get_result())[0];
							if($credits < $penalty)
								$penalty = $credits;
							...
							$query->execute();
							echo '<script>
									document.getElementById("error").innerHTML += "A penalty of $'.$penalty.' was charged for keeping book even after the due date.<br />";
